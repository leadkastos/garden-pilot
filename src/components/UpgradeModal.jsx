  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4" onClick={() => setOpen(false)}>
      <div className="bg-parchment rounded-2xl max-w-md w-full p-8 text-center shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <a href={PAY_URL} className="block w-full bg-garden-500 hover:bg-garden-600 text-white font-semibold py-3 rounded-xl transition-colors mb-3">Upgrade Now — $9.95/mo or $99/yr</a>
        <button onClick={() => setOpen(false)} className="text-garden-500 text-sm hover:underline">Continue in read-only mode</button>
      </div>
    </div>
  )
}
