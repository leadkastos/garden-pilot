...
export default function ReportsPage() {
  ...
  useEffect(() => {
    if (!user) return
    generateReport()
  }, [user, selectedRange, customStart, customEnd])

  const generateReport = async () => {
    setLoading(true)
    const { start, end } = getDateRange()

    const [plantsRes, expensesRes, revenueRes, calendarRes, journalRes, bedsRes] = await Promise.all([
    ...
    // Category breakdown
    const categoryBreakdown = plants.reduce((acc, p) => {
    ...
    // Seeds stats
    const plantsWithSeeds = plants.filter(p => (p.seeds_planted || 0) > 0)
    const totalSeeds = plantsWithSeeds.reduce((s, p) => s + (p.seeds_planted || 0), 0)
    const totalSprouted = plantsWithSeeds.reduce((s, p) => s + (p.seeds_sprouted || 0), 0)
    const avgGermRate = totalSeeds > 0 ? Math.round((totalSprouted / totalSeeds) * 100) : 0

    // Harvests in range
    const allHarvests = []
    plants.forEach(p => {
      (p.harvest_log || []).forEach(h => {
        if (inRange(h.date, start, end)) allHarvests.push({ ...h, plantName: p.name })
      })
    })
    ...
    const topHarvestPlants = Object.entries(harvestByPlant).sort((a, b) => b[1] - a[1]).slice(0, 5)
    ...
    setReportData({
      ...
      plants: {
        total: plants.length,
        categoryBreakdown,
        growAgain: plants.filter(p => p.grow_again === true).length,
        skipNext: plants.filter(p => p.grow_again === false).length,
      },
      seeds: { totalSeeds, totalSprouted, avgGermRate },
      harvests: { total: allHarvests.length, weight: totalHarvestWeight, topPlants: topHarvestPlants },
      expenses: { total: totalSpent, seedSpend, supplySpend, list: revenueInRange },
      revenue: { total: totalRevenue, list: revenueInRange },
      calendar: {
        total: eventsInRange.length,
        plantingEvents: eventsInRange.filter(e => e.type === 'plant').length,
        harvestEvents: eventsInRange.filter(e => e.type === 'harvest').length,
        taskEvents: eventsInRange.filter(e => e.type === 'task').length,
      },
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <!DOCTYPE html><html><head><title>Garden Report</title>
      <style>
      ...
      <div class="section"><div class="section-title">🌱 Plants</div>
        <div class="stats-grid">
          <div class="stat"><div class="stat-num">${reportData.plants.total}</div><div class="stat-label">Total plants</div></div>
          <div class="stat"><div class="stat-num">${reportData.seeds.avgGermRate}%</div><div class="stat-label">Germination rate</div></div>
          <div class="stat"><div class="stat-num">${reportData.plants.growAgain}</div><div class="stat-label">Grow again</div></div>
        </div></div>
      <div class="section"><div class="section-title">🥕 Harvests</div>
        <div class="stats-grid">
      ...
      <div class="section"><div class="section-title">💰 Spending & Revenue</div>
        <div class="stats-grid">
          <div class="stat"><div class="stat-num">$${reportData.expenses.total.toFixed(2)}</div><div class="stat-label">Total spent</div></div>
          <div class="stat"><div class="stat-num">$${reportData.revenue.total.toFixed(2)}</div><div class="stat-label">Revenue</div></div>
      ...
      <div class="section"><div class="section-title">📅 Activity</div>
        <div class="stats-grid">
          ...
          {RANGES.map(range => (
            <button key={range.label} onClick={() => setSelectedRange(range)}
              className={`px-3 py-2 rounded-xl text-sm font-medium border transition-all ${
                selectedRange.label === range.label ? 'bg-garden-600 text-white border-garden-600' : 'bg-white text-garden-600 border-garden-200 hover:border-garden-400'
              }`}>
              {range.label}
            </button>
          ))}
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { icon: Sprout,       label: 'Germination',     value: `${reportData.seeds.avgGermRate}%`,  color: 'text-lime-600',   bg: 'bg-lime-100' },
            ...
            {reportData.seeds.totalSeeds > 0 && (
              <div className="p-3 bg-garden-50 rounded-xl">
                ...
                <div className="h-2.5 bg-garden-200 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-garden-400 to-garden-600 rounded-full"
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="text-center p-3 bg-garden-50 rounded-xl">
                ...
                <div className="text-xs text-garden-400 mt-0.5">{reportData.expenses.total > reportData.revenue.total ? 'Net cost' : 'Net profit'}</div>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-3">
                {reportData.journal.entries.map(entry => (
                  <div key={entry.id} className="p-3 bg-garden-50 rounded-xl border-l-4 border-l-garden-400">
                    <p className="text-xs text-garden-500 mb-1">{entry.date_display} · {entry.time}</p>
